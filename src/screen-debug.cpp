...
namespace ScreenDebug {
    ...
    lv_obj_t* init(DisplayManager::debug_styles_t* de) {
        ...
        insert_by_motor(screen, elements.motor_torque, NULL, motor_offsets, 100, -110, 409, "???.?%%");
    ...
    void update(DataManager::car_data_t data) {
        ...
        if ((data.tv_msg != lastdata.tv_msg) || (data.tc_msg != lastdata.tc_msg)) {
            lv_label_set_text_fmt(elements.tc_status, (data.tv_msg == data.tc_msg ? "TC: -" : "TC: active"));
        }
        ...
        if(data.bms_cellvoltages_min != lastdata.bms_cellvoltages_min) {
            lv_label_set_text_fmt(elements.bms_cellvoltage_min_label, "%1.2f V", data.bms_cellvoltages_min);
        }
